const patchedStep = (responseStep, currentUser) => {
  ...
  let projectIndex = newProjects.findIndex(
    p => p.id === responseStep.task.project.id
  )
...
const destroyedStep = (givenStep, currentUser) => {
  let newProjects = [...currentUser.projects]
  let projectIndex = newProjects.findIndex(
    p => p.id === givenStep.task.project.id
  )
  let newTasks = [...newProjects[projectIndex].tasks]
...
const postedProjectTaskStep = (
  ...
  return [
    ...
    { ...givenProject, tasks: [{ ...givenTask, steps: [givenStep] }] }
  ]
}
